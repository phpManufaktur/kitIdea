...
class dbIdeaProjectAccess extends dbConnectLE {
  ...
  const FIELD_DESCRIPTION = 'access_description';
  ...
  public function __construct($create_table = false) {
    ...
    $this->addFieldDefinition(self::FIELD_RIGHTS, "BIGINT UNSIGNED NOT NULL DEFAULT '0'");
    ...
    switch ($config[self::field_type]) :
      ...
      case self::type_email :
        if ($this->validateEMail($new_value)) {
          ...
          $this->setMessage($this->lang->translate('<p>The email address <b>{{ email }}</b> is not valid!</p>', array(
              'email' => $new_value)));
          return false;
        }
        break;
